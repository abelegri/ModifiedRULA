...
class RightWristTwist : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_right_wrist_twist)
    }
    ...
    fun onRadioButtonClicked(view: View){
        if (view is RadioButton){
            ...
            when (view.getId()){
                R.id.rWristTwistRadioMidRange ->
                    if (checked) {
                        GlobalVariables.rWristTwistScore = 1;
                    }
                R.id.rWristTwistRadioEndRange ->
                    if (checked) {
                        GlobalVariables.rWristTwistScore = 2;
                    }
            }
        }
    }

    fun toRightArmMuscle(view: View){
         ...
         * an overall right arm and wrist posture score. Table A is found with the other
         ...
         *  return a 17. */
        val rowIndex = (GlobalVariables.rUpperArmScore) * 3 + (GlobalVariables.rLowerArmScore) - 4
         ...
         *  would return 7. */
        val columnIndex = (GlobalVariables.rWristScore) * 2 + (GlobalVariables.rWristTwistScore) - 3

        /** this will use the calculated indices to get the overall arm and wrist posture score from
         *  table A */
        GlobalVariables.rArmAndWristPostureScore = GlobalVariables.tableA[rowIndex][columnIndex]

        startActivity(intent)
    }

    fun backToRightWristAdjustment(view: View){
